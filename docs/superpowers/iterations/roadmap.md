...
**Status:** done — all 12 stories complete (see requirements/EPIC-*.md), JOURNEY-0001 closed 2026-09-23 with a live run against a real account: 20 assets downloaded with correct filenames, correct sizes, and manifest rows recorded, process exited clean. See iteration-log.md for full detail, including two real bugs found and fixed during the live proof (a 2FA session-trust header gap, and a filename-decoding bug) and one AC narrowed by live evidence (STORY-0019's checksum verification, since Apple's fileChecksum turned out to be a proprietary MMCS content hash, not a reproducible digest — confirmed independently by another mature open-source project that also marked it "unknown").
